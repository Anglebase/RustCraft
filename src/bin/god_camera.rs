...
fn render_loop() {
    ...
    let view = CAMERA_SYSTEM.view_matrix();
    ...
    if App::fps() < 144.0 {
        warn!("render::loop()", "FPS: {}", App::fps());
    } else if App::fps() < 360.0 {
        info!("render::loop()", "FPS: {}", App::fps());
    }
}

fn main() {
    Log::set_level(Level::Debug);
    let mut app = AppBuilder::new(1000, 600, "RustCraft")
